...
angular.module( 'publipostageClientApp' )
    .controller( 'destinatairesCtrl',
                 [ '$scope', '$filter', 'security', 'Regroupements', '$rootScope', 'MessageService', 'Redirect',
                   ...
                   function ( $scope, $filter, security, Regroupements, $rootScope, MessageService, Redirect,
                       ...
                       $scope.toggle_all_checked = function( value, force_all ) {
                           ...
                           if ( $state.params.type === 'ecrire_personnels' ) {
                               data = force_all ? $scope.data : $filter('filter')( $scope.data, $scope.searchText );
                           } else {
                               data = force_all ? $scope.data : $filter('filter')( $scope.data, { libelle: $scope.searchText } );
                           }

                           _.chain( data )
                               .select( function( item ) {
                                   return $state.params.type === 'ecrire_personnels' || $scope.filter_regroupements()( item );
                               } )
                               .each( function ( item ) {
                                   item.checked = value;
                               } );
                   ...
                   $scope.addRemoveDestination = function ( item ) {
